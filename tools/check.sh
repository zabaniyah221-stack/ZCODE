...
echo "[1/7] Verify Ace 1.44.0 bundled (offline-first, ASLI bukan stub)"
...
echo "[2/7] Verify no unverified SSL (S-22)"
...
echo "[3/7] Verify taskAffinity + singleTop + allowBackup (C-50/S-27/S-21)"
...
echo "[4/7] Verify Topbar faded grey + three lines ≡ (user request)"
...
echo "[5/7] Verify Execution guards (S-18)"
...
grep -q '"kill", "-INT"' app/src/main/java/com/zaba/zcode/core/execution/ExecutionEngine.kt && echo "✅ SIGINT asli (bukan SIGKILL)" || (echo "❌ SIGINT real missing" && exit 1)

echo "[6/7] Verify Fase 1/2 wiring (WebView asli, VM, terminal, pip, themes)"
...
echo "[7/7] Run Python strict tests (Fase 0 + Fase 1/2)"
